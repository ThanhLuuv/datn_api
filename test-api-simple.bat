...
echo.
echo 3. Testing upload image endpoint...
curl -X POST -F "imageFile=@test.png" http://localhost:5256/api/Book/test-upload-image

echo.
echo.
echo 4. Testing create book with image...
curl -X POST -F "isbn=978-604-1-00003-24" -F "title=Test Book" -F "categoryId=2" -F "publisherId=1" -F "unitPrice=100000" -F "publishYear=2025" -F "pageCount=200" -F "authorIds=1" -F "imageFile=@test.png" http://localhost:5256/api/Book
