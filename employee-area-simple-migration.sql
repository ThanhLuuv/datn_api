-- Simple migration script for Employee-Area many-to-many relationship
-- Run this script directly in MySQL Workbench or command line

-- 1. Check if required tables exist, create if not
...
-- 2. Create employee_area table
CREATE TABLE IF NOT EXISTS employee_area (
    ...
    -- Foreign key constraints (only if referenced tables exist)
...
-- 3. Create indexes for performance (MySQL doesn't support IF NOT EXISTS for indexes)
-- Check if indexes exist before creating
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS 
     WHERE table_schema = DATABASE() 
     AND table_name = 'employee_area' 
     AND index_name = 'idx_employee_area_employee_id') = 0,
    'CREATE INDEX idx_employee_area_employee_id ON employee_area(employee_id)',
    'SELECT "Index idx_employee_area_employee_id already exists" as message'
));
...
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS 
     WHERE table_schema = DATABASE() 
     AND table_name = 'employee_area' 
     AND index_name = 'idx_employee_area_area_id') = 0,
    'CREATE INDEX idx_employee_area_area_id ON employee_area(area_id)',
    'SELECT "Index idx_employee_area_area_id already exists" as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = (SELECT IF(
    (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS 
     WHERE table_schema = DATABASE() 
     AND table_name = 'employee_area' 
     AND index_name = 'idx_employee_area_active') = 0,
    'CREATE INDEX idx_employee_area_active ON employee_area(is_active)',
    'SELECT "Index idx_employee_area_active already exists" as message'
));
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- 4. Insert sample area data (all 168 areas from KhuVuc)
...
-- 5. Insert sample employee data (if not exists)
INSERT IGNORE INTO employee (employee_id, account_id, department_id, first_name, last_name, gender, phone, email) VALUES
(1, 1, 1, 'Nguyen', 'Van A', 1, '0901111111', 'shipper1@example.com'),
(2, 2, 1, 'Tran', 'Van B', 1, '0902222222', 'shipper2@example.com'),
(3, 3, 1, 'Le', 'Thi C', 2, '0903333333', 'shipper3@example.com');

-- 6. Insert sample employee-area assignments
INSERT IGNORE INTO employee_area (employee_id, area_id, assigned_at, is_active) VALUES
(1, 1, NOW(), TRUE),   -- Nguyen Van A -> Phường Vũng Tàu
(1, 2, NOW(), TRUE),   -- Nguyen Van A -> Phường Tam Thắng
(1, 3, NOW(), TRUE),   -- Nguyen Van A -> Phường Rạch Dừa
(1, 63, NOW(), TRUE),  -- Nguyen Van A -> Phường Sài Gòn
(1, 64, NOW(), TRUE),  -- Nguyen Van A -> Phường Tân Định

(2, 4, NOW(), TRUE),   -- Tran Van B -> Phường Phước Thắng
(2, 5, NOW(), TRUE),   -- Tran Van B -> Phường Bà Rịa
(2, 6, NOW(), TRUE),   -- Tran Van B -> Phường Long Hương
(2, 65, NOW(), TRUE),  -- Tran Van B -> Phường Bến Thành
(2, 66, NOW(), TRUE),  -- Tran Van B -> Phường Cầu Ông Lãnh

(3, 7, NOW(), TRUE),   -- Le Thi C -> Phường Phú Mỹ
(3, 8, NOW(), TRUE),   -- Le Thi C -> Phường Tam Long
(3, 9, NOW(), TRUE),   -- Le Thi C -> Phường Tân Thành
(3, 67, NOW(), TRUE),  -- Le Thi C -> Phường Bàn Cờ
(3, 68, NOW(), TRUE);  -- Le Thi C -> Phường Xuân Hoà

-- 7. Verify the data
SELECT 'Areas created:' as info, COUNT(*) as count FROM area;
SELECT 'Employees created:' as info, COUNT(*) as count FROM employee;
SELECT 'Employee-Area assignments:' as info, COUNT(*) as count FROM employee_area;
